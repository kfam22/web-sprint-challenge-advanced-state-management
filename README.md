...
Context API helps to solve the problem of excessive prop drilling and overhead of setting up redux by allowing developers to connect variables at the global level.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

actions: includes instructions on how to change state based on the specific action type.  actions can include payload properties that can be applied to the state change, but aren't required.

reducers: functions that return state data which is triggered by action types, usually in the form of a switch statement with cases for each action type

store: stores an applications state data.

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?

Redux thunk allows us to call actions that return functions rather than action objects and intercepts the redux flow to perform an async operation before the returned info gets to the reducer. The function gets the dispatch method as a parameter which is used to dispatch regular actions in the function after an async operation takes place. 

4. What is your favorite state management system you've learned and this sprint? Please explain why!

I enjoyed Redux because I like the way that all portions of the process are organized into a clear flow.  The organizational aspect of it helped me easily read my code and trace back my steps when I encountered bugs.
